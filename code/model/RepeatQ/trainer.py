...
from defs import PAD_TOKEN, TRAINED_MODELS_DIR, EOS_TOKEN
...
class RepeatQTrainer:
        ...
        def train(nb_epochs, data, dev_data, ds_type):
            ...
            for epoch in range(nb_epochs):
                for features, label in tqdm(data):
                    self.train_step(features, label, epoch=epoch, ds_type=ds_type)

                dev_score = self.dev_step(dev_data)
                tf.print("Score on dev set:", dev_score)
                if self.config.saving_model:
                    checkpoint_filename = f"{model_save_dir}/{ds_type}_epoch_{epoch + 1}_bleu_{'%.2f' % dev_score}"
    ...
    @tf.function
    def train_step(self, features, labels, epoch, ds_type):
    ...
    def dev_step(self, dev_data):
        ...
        for i, (features, label) in tqdm(enumerate(dev_data)):
            predictions, _ = self.model.get_actions(features, target=label, training=False)
            paddings = (
                (0, 0), (0, tf.math.maximum(0, self.config.max_generated_question_length - tf.shape(predictions)[1]))
            )
            predictions_padded = tf.pad(
                predictions,
                paddings=paddings,
                mode="CONSTANT"
            )
            predicted_questions = predicted_questions.write(i, predictions_padded)
        ...
        def compute_bleu(refs, hyps):
            refs = [[RepeatQTrainer.make_sequence(ref)] for ref in refs.numpy()]
            hyps = [RepeatQTrainer.make_sequence(hyp) for hyp in hyps.numpy()]
            for i in range(len(hyps)):
                tf.print("Ref:", self._tokens_to_sentence(refs[i][0]))
                tf.print("Hyp:", self._tokens_to_sentence(hyps[i]), "\n")
    ...
    def _supervised_step(self, features, target, loss_fc=tf.keras.losses.SparseCategoricalCrossentropy(reduction=tf.keras.losses.Reduction.NONE)):
        with tf.GradientTape() as tape:
            predictions, pointer_softmax = self.model.get_actions(features, target=target, training=True)
    ...
    def _reinforce_step(self, features, targets, environment):
        ...
        with tf.GradientTape() as tape:
            predictions, logits = self.model.get_actions(features, beams, training=True)
            tf.py_function(
                self._debug_output,
            ...
            loss = self._policy_gradient_loss(
                actions=predictions, targets=targets, features=features, environment=environment, logits=logits
            )
        return loss, tape
    ...
    def _tokens_to_sentence(self, tokens):
        return " ".join(self.reverse_voc[int(t)] for t in tokens)
    ...
    def make_sequence(tokens):
        no_pad = [str(t) for t in tokens if t != PAD_TOKEN]
        try:
            return no_pad[:no_pad.index(str(EOS_TOKEN)) + 1]
        except ValueError:
            pass
        return no_pad
